...
//}
    ...
    foreach ($workers as $worker)
    {
        ...
        {
            case "junior":
            $new_jun = new junior($worker['id'],$worker['fio'],$worker['date_recruitment'],$worker['zp'],$worker['parent_id']);
            array_push($juniors,$new_jun);
            break;
            case "middle":
            $new_mid = new middle($worker['id'],$worker['fio'],$worker['date_recruitment'],$worker['zp'],$worker['parent_id']);
            array_push($middles,$new_mid);
            break;
            case "senior":
            $new_sen = new senior($worker['id'],$worker['fio'],$worker['date_recruitment'],$worker['zp'],$worker['parent_id']);
            array_push($seniors,$new_sen);
            break;
            case "team_lead":
            $new_lead = new team_lead($worker['id'],$worker['fio'],$worker['date_recruitment'],$worker['zp'],$worker['parent_id']);
